...
async function data(ctx, cmd) {
	...
	/* more handling */
	if (
		Catch( !clan.ops.includes(ctx.author.id) && clan.owner != ctx.author.id, { text: "You have to have operator to kick users." }) ||
		Catch( clan.ops.includes(user.id) && clan.owner != ctx.author.id, { text: "Only the owner can kick operators." }) ||
		Catch( clan.owner == user.id, { text: "You can't kick the owner of the clan." }) ||
        Catch( ctx.author.id == user.id, { text: "You can't yourself." })
	) return;

	
	const embed = new psc.Embed({
		description: `**${emojis.success}  Kicked *<@${user.id}>* | *${clan.name}***`,
		footer: `( id: ${clan.id} )`,
...
psc.command({ name: "kick", cooldown: "3s"}, data);
